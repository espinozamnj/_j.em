window.addEventListener('load', function(){
  ...
  $('#update-note').addEventListener('click', function(){
    var this_note_to_update = db.collection("notes").doc(last_note_id)
  ...
  db.collection("todo").onSnapshot((querySnapshot) => {
    ...
    saved_order.get().then((doc) => {
      if (doc.exists) {
        ...
        if (last_order_todo.length == 0) {
          last_order_todo = JSON.parse(jss(clone_local_order_only_names))
        }
        tbt.innerHTML = ''
        tbt.appendChild(frg)
      }
      ...
      $$('[id^=cont]').forEach(function(d){
        if (d.id.includes(clss)) {
          ...
          if (d.id.includes('note')) {
            show_note(last_note_id)
            $('#title').removeAttribute('disabled')
            $('.header').setAttribute('data-note-status', 'true')
          } else {
            $('#title').value = 'Firenote - ' + clss
