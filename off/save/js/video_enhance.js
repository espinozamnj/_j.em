// HTML5播放器增强插件 - 修订版
// https://github.com/xxxily/h5player/blob/master/src/h5player.js
...
var videoCapturer = {
    ...
    download (canvas, title, video) {
      ...
      try {
        ...
        window.alert('The video source is restricted by the CORS logo, and screenshots cannot be downloaded');
  ...
  (function () {
    ...
    const h5Player = {
      ...
      setPlaybackRate: function (num, notips) {
        ...
        !notips && t.tips('speed: ' + player.playbackRate + 'x')
      },
      /**
       * 初始化自动播放逻辑
      ...
      keyList: ['enter', 'shift', 'control', 'alt', 'escape', ' ', 'arrowleft', 'arrowright', 'arrowright', 'arrowup', 'arrowdown', '1', '2', '3', '4', 'c', 'd', 'e', 'f', 'i', 'j', 'k', 'o', 'p', 'q', 'r', 's', 't', 'n', 'u', 'w', 'x', 'y', 'z', '\\', '|'],
      keyMap: {
        enter: 13,
        shift: 16,
        ctrl: 17,
      ...
      palyerTrigger: function (player, event) {
        ...
        if (event.shiftKey && !event.ctrlKey && !event.altKey) {
          ...
          let scale = t.scale = Number(t.scale).toFixed(2)
          scale = Math.floor(scale * 100) / 100
          let last_rotate = player.style.transform.match(/rotate[^ ]+/)
          last_rotate == null || undefined ? last_rotate = '' : last_rotate = last_rotate
          let style_trn = `scale(${scale}) translate(${t.translate.x}px, ${t.translate.y}px) ${last_rotate}`
          player.style.transform = style_trn
          let scale_text = scale * 100
          scale_text= Math.floor(scale_text * 1) / 1
          let tipsMsg = `scale ${scale_text}%`
          if (t.translate.x) {
            tipsMsg += `, moveX: ${t.translate.x}px`
          }
          if (t.translate.y) {
            tipsMsg += `, moveY: ${t.translate.y}px`
          }
          t.tips(tipsMsg)
        ...
        // 方向键左←：后退3秒
        if (keyCode === 37 && !event.ctrlKey) {
          t.setCurrentTime(-t.skipStep)
        }
        if (keyCode === 39 && event.ctrlKey) {
          t.setCurrentTime(t.skipStep * 3)
        }
        // 方向键左←：后退3秒
        if (keyCode === 37 && event.ctrlKey) {
          t.setCurrentTime(-t.skipStep * 3)
        ...
        */
        ...
        if (keyCode === 98 && event.ctrlKey) {
          t.setPlaybackRate(1.25)
        }
        if (keyCode === 99 && event.ctrlKey) {
          t.setPlaybackRate(1.5)
        }
        // 按键F：下一帧
        if (keyCode === 70 && !event.ctrlKey) {
        ...
        // 按键E：亮度增加%
        if (keyCode === 69 && !event.ctrlKey) {
          t.filter.key[0] += 0.1
          t.filter.key[0] = t.filter.key[0].toFixed(2)
          t.filter.setup()
          t.tips('brightness: ' + parseInt(t.filter.key[0] * 100) + '%')
        }
        // 按键W：亮度减少%
        if (keyCode === 87 && !event.ctrlKey) {
          if (t.filter.key[0] > 0) {
            t.filter.key[0] -= 0.1
            t.filter.key[0] = t.filter.key[0].toFixed(2)
            t.filter.setup()
        ...
        // 按键T：对比度增加%
        if (keyCode === 84 && !event.ctrlKey) {
          t.filter.key[1] += 0.1
          t.filter.key[1] = t.filter.key[1].toFixed(2)
          t.filter.setup()
          t.tips('contrast: ' + parseInt(t.filter.key[1] * 100) + '%')
        }
        // 按键R：对比度减少%
        if (keyCode === 82 && !event.ctrlKey) {
          if (t.filter.key[1] > 0) {
            t.filter.key[1] -= 0.1
            t.filter.key[1] = t.filter.key[1].toFixed(2)
            t.filter.setup()
        ...
        // 按键U：饱和度增加%
        if (keyCode === 85 && !event.ctrlKey) {
          t.filter.key[2] += 0.1
          t.filter.key[2] = t.filter.key[2].toFixed(2)
          t.filter.setup()
          t.tips('saturate: ' + parseInt(t.filter.key[2] * 100) + '%')
        }
        // 按键Y：饱和度减少%
        ...
        if (keyCode === 79 && !event.ctrlKey) {
          t.filter.key[3] += 1
          t.filter.setup()
          t.tips('hue: ' + t.filter.key[3] + 'º')
        }
        // 按键I：色相减少 1 º
        if (keyCode === 73 && !event.ctrlKey) {
          t.filter.key[3] -= 1
          t.filter.setup()
          t.tips('hue: ' + t.filter.key[3] + 'º')
        }
        ...
        // 按键K：模糊增加 1 px
        if (keyCode === 75 && !event.ctrlKey) {
          t.filter.key[4] += 1
          t.filter.setup()
          t.tips('blur: ' + t.filter.key[4] + 'px')
        }
        // 按键J：模糊减少 1 px
        if (keyCode === 74 && !event.ctrlKey) {
          if (t.filter.key[4] > 0) {
            t.filter.key[4] -= 1
            t.filter.setup()
          }
        ...
        if (keyCode === 83 && !event.ctrlKey) {
          ...
          let att = player.style.transform.split(') ')
          let n_att = []
          if (att != '') {
              att.forEach(function (f) {
                  if (!f.includes('rotate')) {
                      let a = f.replace(')', '')
                      n_att.push(a + ')')
                  }
              })
        ...
        //   att.join(' ')
          player.style.transform = 'rotate(' + t.rotate + 'deg) ' + n_att.join(' ')
          t.tips('rotate: ' + t.rotate + 'º')
        }
      ...
      keydownEvent: function (event) {
        ...
        if (!t.enable) {
          console.log('h5Player change status')
          return false
        }
